...
export function QueryTabs({ selectedConnection, activeTab, onActiveTabChange, tabs, onTabsChange, onAddTab }: QueryTabsProps) {
	const addTab = () => {
		if (onAddTab) {
			onAddTab()
		} else {
	...
	const updateTabState = (tabId: string, newState: QueryState) => {
		onTabsChange(tabs.map((tab) => (tab.id === tabId ? { ...tab, queryState: newState } : tab)))
	}
			...
			<Box className="flex items-center border-b">
				<Tabs.List className="flex-1">
					{tabs.map((tab) => (
						...
						>
							<p className="text-xs">{tab.queryState.queryText}</p>
							{tabs.length > 1 && (
								<Button
									variant="ghost"
									size="sm"
